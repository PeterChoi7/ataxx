...
class Board {
    ...
    /** Number of consecutive non-extending moves before game ends. */
    static final int MIN_BOARD = 24;

    /** Number of consecutive non-extending moves before game ends. */
    static final int MAX_BOARD = 96;
    ...
    void clear() {
        ...
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 11; j++) {
                int currPosition = (i) * EXTENDED_SIDE + (j);
                if (i == 0 || i == 1
                        || i == 9 || i == 10) {
                    set(currPosition, BLOCKED);
                } else if (j == 0 || j == 1
                        || j == 9 || j == 10) {
                    set(currPosition, BLOCKED);
                } else {
                    set(currPosition, EMPTY);
                }
            }
        }
        set(index('a', '1'), BLUE);
    ...
    /** Determine the winner as the game is played. */
    void getWinnerMethod() {
    ...
    boolean legalMove(Move move) {
        if (move == null) {
            ...
            int initialPosition = index(move.col0(), move.row0());
            int checkPosition = index(move.col1(), move.row1());
            if (_board[initialPosition] != whoseMove()) {
                return false;
            }
            if (move.isExtend() || move.isJump()) {
                if (_board[checkPosition] == EMPTY) {
                    return true;
                }
            }
            if (getWinner() != null) {
    ...
    boolean canMove(PieceColor who) {
        for (int i = MIN_BOARD; i <= MAX_BOARD; i++) {
            if (get(i) == who) {
                for (int r = -2; r < 3; r++) {
                    for (int c = -2; c < 3; c++) {
                        if (_board[neighbor(i, r, c)] == EMPTY) {
                            return true;
                        }
                    }
                }
    ...
    void makeMove(Move move) {
        ...
        int initialPosition = index(move.col0(), move.row0());
        int finalPosition = index(move.col1(), move.row1());

        for (int r = -1; r < 2; r++) {
            for (int c = -1; c < 2; c++) {
                if (_board[neighbor(finalPosition, r, c)] == opponent) {
                    set(neighbor(finalPosition, r, c), whoseMove());
        ...
        if (move.isJump()) {
            set(initialPosition, EMPTY);
            set(finalPosition, _whoseMove);
            _numMoves += 1;
            _numJumps += 1;
        } else {
            set(finalPosition, _whoseMove);
    ...
    void undo() {
        while (_undoSquares.peek() != null && _undoPieces.peek() != null) {
            ...
            PieceColor ogColor = get(location);

            _board[location] = color;
            if (color == RED) {
                if (ogColor == BLUE) {
                    incrPieces(RED, 1);
                    incrPieces(BLUE, -1);
                } else if (ogColor == EMPTY) {
                    incrPieces(RED, 1);
                }
            } else if (color == BLUE) {
                if (ogColor == RED) {
    ...
    void setBlock(char c, char r) {
        ...
        while (legalBlock(c, r)) {
            char refX = (char) ('1' + '7' - r);
            char refY = (char) ('a' + 'g' - c);
            if (c == 'd' && r == '4') {
                set(index(c, r), BLOCKED);
                _totalOpen -= 1;
            } else if (c == 'd') {
                set(index(c, r), BLOCKED);
                set(index(c, refX), BLOCKED);
                _totalOpen -= 2;
            } else if (r == 4) {
                set(index(c, r), BLOCKED);
                set(index(refY, r), BLOCKED);
                _totalOpen -= 2;
            } else {
                set(index(c, r), BLOCKED);
                set(index(c, refX), BLOCKED);
                set(index(refY, r), BLOCKED);
                set(index(refY, refX), BLOCKED);
                _totalOpen -= 4;
            }
        }
